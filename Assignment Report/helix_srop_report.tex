...
LLM and agents & Google ADK (\texttt{google-adk}) with Gemini 2.0 Flash \\
Embeddings & Google Generative AI (\texttt{text-embedding-004}, 768 dimensions) \\
...
Each chunk text is embedded via \texttt{google.generativeai.embed\_content} using the \texttt{models/text-embedding-004} model, which produces 768-dimensional vectors. The chunk, its embedding, its metadata, and its ID are upserted into the \texttt{helix\_docs} ChromaDB collection at \texttt{settings.CHROMA\_PATH}.
...
Embedding model & \texttt{text-embedding-004} (768 dims) & Native to Google AI ecosystem; consistent with LLM provider \\
